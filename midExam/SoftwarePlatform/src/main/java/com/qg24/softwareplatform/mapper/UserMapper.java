...
public interface UserMapper {
    ...
    @Select("select count(*) from software where userId=#{userId}")
    ...
    @Select("select * from user_software_auth where userId=#{userId}")
    List<UserSoftwareAuth> getAvailableSoftware(String userId);//查询用户授权过的软件

    @Select("select softwareName from software where softwareId=#{softwareId}")
